...
//
//  2) @Value 어노테이션이 붙은 필드에 값 삽입
//     ncpObjectStorageService.getProperty("ncp.end-point");
//
//  3) @PostConstruct 어노테이션이 붙은 메서드 실행
//     ncpObjectStorageService.init();


  @PostConstruct
  public void init() {
    System.out.println("init() 호출 됨! : " + endPoint);

    // AWS S3 API 버전 1에서 발생하는 경고 메시지 무시
    // 버전 2로 변경하면 경고 메시지가 발생하지 않는다
    // 단, 버전 2는 네이버 클라우드의 Object Storage 서비스와의 연동을 지원하지 않는다
